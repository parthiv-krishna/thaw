...
class MachineMonitor:
    def __init__(self, machines_config):
        ...
        self.status_cache = {}  # {machine_name: {'status': 'awake/asleep', 'timestamp': time.time()}}
    ...
    def wake_on_lan(self, mac_address, broadcast_ip, port=9):
        ...
        try:
            ...
            logger.info(f"Wake-on-LAN packet sent to {mac_address} at {broadcast_ip}:{port}")
            return True
        ...
        try:
            ...
            if machine_name in self.status_cache:
                ...
                if old_status and old_status != status:
                    logger.info(f"{machine_name} status changed: {old_status} -> {status}")
        ...
        for name, config in machines.items():
            ...
            for field in required_fields:
                if field not in config:
                    raise ValueError(f"Missing required field '{field}' for machine '{name}'")
